...
client.handlers.packethub = {
    ...
    setnewexpirytoasset: function (assetId, expiryList) {
        ...
        logger.log("assetId: "+JSON.stringify(assetId) );
        logger.log("expiryList: "+JSON.stringify(expiryList) );
    },

    updatetradesatexpiry: function () {
        io.emit("TradeAsset", {type: "TRADE_ASSET.SHOULD-UPDATE"});
        console.log("should update trades" );
        logger.log("should update trades" );
    },
...
client.serviceHandlers.onUnauthorized = (res) => {
    ...
    let result = httpsServer.get(location, function (loginResult) {
...
client.serviceHandlers = {
    ...
    onerror: (error)=> {
        ...
        logger.log("Websocket onerror: ", error);
    },

    bindingError: (error) => {
        console.log("Websocket bindingError: ", error);
        logger.log("Websocket bindingError: ", error);
    },
    ...
    reconnecting: (retry /* { inital: true/false, count: 0} */) => {
        ...
        logger.log("Websocket Retrying: ", retry);
...
redisClient.get("AssetsDescription", (err, reply) => {
    ...
    logger.log("-----");
    logger.log(JSON.stringify(assetsDescription));
});
    ...
    .done(function (err, result) {
        if (!err)  {
            if(result){
                ...
                logger.log("connected to packet hub", result);
            }
            else
            {
                console.log("failed connecting to packet hub - user not authenticated", result);
                logger.log("failed connecting to packet hub - user not authenticated", result);
            }
        }
    });
};
    ...
    .done(function (err, result) {
        if (!err)  {
            if(result){
                ...
                logger.log("connected to balance hub", result, token);
            }
            else {
                console.log("failed connecting to balance hub - user not authenticated", token);
                logger.log("failed connecting to balance hub - user not authenticated", token);
            }
        }
    });
};
